...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $plocationtype = $_POST['locationtype']; // New field

        // Basic validation for PackageDuration and MaxSlots
        if ($pduration <= 0) {
            $error = "Package Duration must be greater than 0.";
        } elseif ($pmaxslots < 0) {
            $error = "Maximum Slots cannot be negative.";
        } else {
            // Handle file upload
            move_uploaded_file($_FILES["packageimage"]["tmp_name"], "pacakgeimages/" . $_FILES["packageimage"]["name"]);

            // Updated SQL query to include locationType
            $sql = "INSERT INTO tbltourpackages(PackageName, PackageType, PackageLocation, PackagePrice, PackageFetures, PackageDetails, PackageImage, PackageDuration, MaxSlots, locationType) VALUES(:pname, :ptype, :plocation, :pprice, :pfeatures, :pdetails, :pimage, :pduration, :pmaxslots, :plocationtype)";
            ...
            $query->bindParam(':plocationtype', $plocationtype, PDO::PARAM_STR); // New binding
                        ...
                        <div class="tab-pane active" id="horizontal-form">
                            <form class="form-horizontal" name="package" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="number" class="form-control1" name="packageduration" id="packageduration" placeholder="Package Duration in Days (e.g., 3)" min="1" required>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="focusedinput" class="col-sm-2 control-label">Maximum Slots</label>
                                    <div class="col-sm-8">
                                        <input type="number" class="form-control1" name="maxslots" id="maxslots" placeholder="Maximum Slots (e.g., 10)" min="0" required>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="focusedinput" class="col-sm-2 control-label">Location Type</label> <!-- New field -->
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control1" name="locationtype" id="locationtype" placeholder="Location Type (e.g., Mountain, Beach)" required>
                                    </div>
                                </div>
                                <div class="row">
